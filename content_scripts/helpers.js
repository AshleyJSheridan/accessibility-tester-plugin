function addBodyFilter(filterName, filterOptions) {
    var existingFilters = document.body.style.filter;
    if (!existingFilters.includes(filterName)) {
        var newFilter = existingFilters + " " + filterName + "(" + filterOptions + ")";
...
function removeBodyFilter(filterName) {
    ...
    if (existingFilters.includes(filterName)) {
        ...
        var newFilterList = existingFilters.replace(filterRegex, "");
...
function applySvgColorMatrixFilter(matrix, filterName) {
    var filterId = "filter_" + filterName;
    var svgFilterHtml = "<svg height=\"0\">\n\t\t<filter id=\"" + filterId + "\">\n\t\t\t<feColorMatrix values=\"" + matrix + "\"/>\n\t\t</filter>\n\t</svg>";
    addHtmlWithIdIfNotPresent(filterId, svgFilterHtml);
    removeBodyFilter("url");
    addBodyFilter("url", "#" + filterId);
}
function showFailures(failureList, failureMessage, successOnEmpty) {
    if (successOnEmpty === void 0) { successOnEmpty = true; }
...
function getHeadingLevel(heading) {
    var headingString = heading.tagName.toLowerCase();
    return parseInt(headingString.substring(1));
}
function getMultimediaElementsWithoutTextTracks(mediaType) {
    var multimediaElements = document.querySelectorAll(mediaType);
    var failingElements = [];
    for (i = 0; i < multimediaElements.length; i++) {
        var multimediaElement = multimediaElements[i];
        var track = multimediaElement.querySelectorAll("track[kind=subtitles], track[kind=captions], track[kind=descriptions]");
        if (track.length === 0) {
...
function getMultimediaElementsWithAutoplay(mediaType) {
    var multimediaElements = document.querySelectorAll(mediaType + "[autoplay]");
    return Array.from(multimediaElements);
}
function getElementWithAttributeValue(elementType, attribute, attributeValue) {
    if (attributeValue === null)
        return false;
    var element = document.querySelector(elementType + "[" + attribute + "=" + attributeValue + "]");
    return element;
}
function getCousinOfType(node, parentType, childType) {
    var parentNode = getParentOfType(node, parentType);
    var childNode = getDescendantOfType(parentNode, childType);
...
function doesTextContrast(fontSize, fontWeight, contrast) {
    var minContrastLevel = 4.5;
    var largeTextContrastLevel = 3;
    var largeTextSize = 24;
    var largeTextSizeBold = 18.66;
    var boldTextWeight = 700;
    var passedMinContrast = contrast >= minContrastLevel;
    var isLargeText = (fontSize >= largeTextSize) || (fontWeight >= boldTextWeight && fontSize >= largeTextSizeBold);
    var passedLargeTextMinContrast = (isLargeText && contrast >= largeTextContrastLevel);
    return passedMinContrast || passedLargeTextMinContrast;
}
function getColourFromComputed(computedStyles, colourProperty) {
    var colour = getPropertyFromComputedStyles(computedStyles, colourProperty);
...
function getComputedStyleForNode(node, property) {
    var computedStyles = getComputedStylesForNode(node);
...
function hasDescdendantsOfTypeWithoutGivenComputedStyle(node, descendantSelector, computedStyleProperty, matchingValues) {
    var children = node.querySelectorAll(descendantSelector);
    for (var i = 0; i < children.length; i++) {
        var childNode = children[i];
        var computedStyleValue = getComputedStyleForNode(childNode, computedStyleProperty);
...
function getColourContrast(rgb1, rgb2) {
    var luminance1 = getColourLuminance(rgb1.r, rgb1.g, rgb1.b);
    var luminance2 = getColourLuminance(rgb2.r, rgb2.g, rgb2.b);
...
function disableExternalStyles(externalStylesheet) {
    var href = externalStylesheet.getAttribute("href");
...
function enableExternalStyles(externalStylesheet) {
    var href = externalStylesheet.dataset.disabledHref;
    externalStylesheet.setAttribute("href", href);
}
function enableInlineStyle(element) {
    var style = element.dataset.disabledStyle;
...
function enableBlockStyle(element) {
    var style = atob(element.dataset.disabledBlockStyle);
    element.innerHTML = style;
}
function disableBlockStyle(element) {
    var style = btoa(element.innerHTML);
